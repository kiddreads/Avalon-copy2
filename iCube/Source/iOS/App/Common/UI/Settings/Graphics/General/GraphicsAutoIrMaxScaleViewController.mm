#import "GraphicsAutoIrMaxScaleViewController.h"

#import "Core/Config/GraphicsSettings.h"

@implementation GraphicsAutoIrMaxScaleViewController {
  UISlider* _slider;
  UILabel* _valueLabel;
}
...
- (UITableViewCell*)tableView:(UITableView*)tableView cellForRowAtIndexPath:(NSIndexPath*)indexPath {
  static NSString* cellId = @"AutoIrMaxScaleSliderCell";
  UITableViewCell* cell = [tableView dequeueReusableCellWithIdentifier:cellId];
  if (!cell) {
    cell = [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:cellId];
    UILabel* title = cell.textLabel;
    title.text = @"Max Scale";
  ...
  const int current = Config::Get(Config::GFX_AUTO_IR_MAX_SCALE);
...
- (void)valueChanged:(UISlider*)slider {
  ...
  Config::SetBaseOrCurrent(Config::GFX_AUTO_IR_MAX_SCALE, value);
}
