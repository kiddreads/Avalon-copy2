...
final class WebDAVSource: RemoteLibrarySource, Identifiable {
  ...
  init(id: String = UUID().uuidString, name: String, url: URL, username: String?, password: String?, recursive: Bool, interval: TimeInterval = 900, startPath: String? = nil, enablePreCaching: Bool = false) {
    ...
    #if canImport(os)
    Self.logger.info("Initialized WebDAVSource name=\(self.name, privacy: .public) url=\(baseURL.absoluteString, privacy: .public) root=\(rootURL.absoluteString, privacy: .public) preCache=\(self.enablePreCaching)")
    #endif
  }
  ...
  func start() {
    ...
    if onlineCont == nil {
      var oc: AsyncStream<Bool>.Continuation!
      onlineStream = AsyncStream<Bool> { c in oc = c }
      onlineCont = oc
    }
    if itemsCont == nil {
      var ic: AsyncStream<[RemoteLibraryItem]>.Continuation!
      itemsStream = AsyncStream<[RemoteLibraryItem]> { c in ic = c }
      itemsCont = ic
    }
    if scanningProgressCont == nil {
      var pc: AsyncStream<Double>.Continuation!
      scanningProgress = AsyncStream<Double> { c in pc = c }
      scanningProgressCont = pc
    }
  ...
  func stop() {
    #if canImport(os)
    Self.logger.info("Stopping source loop for \(rootURL.absoluteString, privacy: .public)")
    #endif
  ...
  func requestRefresh() {
    #if canImport(os)
    Self.logger.info("requestRefresh called for \(rootURL.absoluteString, privacy: .public)")
    #endif
  ...
  private func loop() async {
    #if canImport(os)
    Self.logger.info("Starting WebDAV loop for \(rootURL.absoluteString, privacy: .public)")
    #endif
    while true {
      ...
      if Task.isCancelled { break }
      #if canImport(os)
      Self.logger.debug("Loop iteration: pinging \(rootURL.absoluteString, privacy: .public)")
      #endif
      await ping()
      if Task.isCancelled { break }
      if isOnline {
  ...
  @MainActor private func ping() async {
    do {
      ...
      #if canImport(os)
      Self.logger.info("Ping \(rootURL.absoluteString, privacy: .public) status=\(isOnline, privacy: .public) reachable=\(serverReachable, privacy: .public) code=\(code, privacy: .public)")
      #endif
    } catch {
      ...
      isOnline = false
      #if canImport(os)
      Self.logger.error("Ping failed for \(rootURL.absoluteString, privacy: .public): \(error.localizedDescription, privacy: .public)")
      #endif
    }
  }
  ...
  @MainActor private func enumerateAndEmit() async {
    #if canImport(os)
    Self.logger.info("Enumerating WebDAV at \(rootURL.absoluteString, privacy: .public) recursive=\(recursive, privacy: .public)")
    #endif
    do {
      ...
      #if canImport(os)
      Self.logger.error("Enumeration error at \(rootURL.absoluteString, privacy: .public): \(error.localizedDescription, privacy: .public)")
      #endif
    }
  }
  ...
  private func enumerateWebDAV() async throws -> [RemoteLibraryItem] {
    #if canImport(os)
    Self.logger.info("Starting WebDAV enumeration at \(rootURL.absoluteString, privacy: .public)")
    #endif
