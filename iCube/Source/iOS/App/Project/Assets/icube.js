// icube.js — StikDebug JIT broker for iCube (iOS 26 TXM).
//
// Shipped inside the app and handed to StikDebug at runtime via the
// `stikdebug://enable-jit?...&script-data=<base64url>` URL scheme (see StikDebugLauncher.swift),
// so users never have to pre-assign a script in StikDebug's Scripts tab.
//
// It is the `brk #0x69` broker that authorizes iCube's dual-mapped JIT region — the same
// protocol StikDebug auto-assigns to DolphiniOS/UTM/Flycast (UTM-Dolphin.js). iCube's C++
// (MemoryUtil_iOS_LuckTXM.cpp) issues `brk #0x69` first, which this script answers with
// prepare_memory_region, so JIT is authorized on the first handshake (no 0xf00d migration).
//
// Derived from StikDebug's UTM-Dolphin.js (github.com/StephenDev0/StikDebug). GPL-2.0-or-later.
