...
| bsnes (SNES) | **integrated, tested** | `BsnesSpec` + `BsnesInputMap.swift`; `BsnesCoreTests` proves load/run/save-state against the compiled, linked core using a minimal LoROM image that satisfies bsnes's own header-scoring heuristic; GPL-3.0-only, the only core here with no libretro-common dependency at all, and the only one requiring the package-wide C++ standard raised to C++17 |
| pcsx_rearmed (PS1) | **integrated, tested** | `PCSXSpec` + `PCSXInputMap.swift`; `PCSXCoreTests` proves load/run/save-state against the compiled, linked core using a minimal PS-EXE homebrew image; GPL-2.0-or-later, DYNAREC=0 on iOS upstream (no JIT needed), real HLE BIOS (no Sony BIOS file required), the only core here with real AArch64 assembly (the GTE) |
