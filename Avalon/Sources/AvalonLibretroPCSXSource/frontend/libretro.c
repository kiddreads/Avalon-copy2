...
#if defined(__MACH__) && !defined(TVOS) && !defined(DRC_DISABLE)
   // magic sauce to make the dynarec work on iOS -- self-attaching via PTRACE_TRACEME tricks the
   // kernel into granting RWX pages without the get-task-allow entitlement, which the dynarec
   // needs and DRC_DISABLE means we never link in. Also genuinely harmful in a sandboxed dev
   // environment: self-tracing an already-supervised process (a build tool's own child-process
   // capture, for example) can stop it outright rather than merely doing nothing -- confirmed
   // directly: this exact test process entered ptrace-stopped state ("T" in ps) here, unrelated
   // to anything about the ROM or the GPU state this test was actually trying to diagnose.
